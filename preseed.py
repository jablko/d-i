...
class CommandSequence(list):
  ...
  def __str__(self):
    ...
    if paths:
      paths = uniquify(paths)
      result += ';chown --recursive 1000:1000 ' + ' '.join(
          shlex.quote(filename) for filename in paths)
    return result
...
class Sed:
  ...
  def __str__(self):
    ...
    return f'echo -e {shlex.quote(script)} | sed --file - --in-place {shlex.quote(self.filename)}'
...
def get_man_packages():
  ...
  if man_packages is None:
    args = ['apt-file', '--package-only', 'search', '/usr/share/man/']
    with subprocess.Popen(args, stdout=subprocess.PIPE) as p:
      with io.TextIOWrapper(p.stdout) as f:
        text = f.read()
